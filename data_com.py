...
class DataCom:
    ...
    def __init__(self, filename, numero_de_pares: int) -> None:
        ...
        self.MAP = []  # IDEIA CIRCULAR P2P: [0, 1], [a,a+1], ... [size-1, 0]
    ...
    def _config_ports(self, filename):  # IDEIA P2P: [50, C1], [S1, C2], [S2, ce]...
        _port = int(-1)
        try:
            with open(filename, 'r') as f:
                _port = int(f.read())  # Le porta usada no servidor
            with open(filename, 'w') as f:
                f.write(str(_port+1))  # Grava Porta usada para cliente conectar ao servidor parceiro
        except IOError:
        ...
        Ant_I = I - 1 if I - 1 >= 0 else self.SIZE - 1  # diminuir, e quando negativo Ant_I
        self.antecessor_name = "NO[{0}]".format(self.MAP[Ant_I][0]*DataCom.FAIXA + DataCom.SPORT)
        self.setF(I)
        return (I)

    def __repr__(self) -> str:
        s = "Servidor({0}), PortServer({1}), SUCESSOR ({2} -> FAIXA [{3}-{4}])....".format(self.HOST_SERVER,
                                                                                   str(self.PORT_SERVER),
                                                                                   str(self.SUCESSOR),
                                                                                   str(self.Fi), str(self.Fj))
    ...
    def setF(self, I: int):
        ...
        if (I + 1 == self.SIZE):
            self.Fi = int(self.PORT_SERVER - DataCom.SPORT + 1)  # Caso Faixa Final até zero